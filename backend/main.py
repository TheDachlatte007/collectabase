...
async def update_game(game_id: int, game: GameUpdate):
    """Update a game - only update fields that are provided"""
    with get_db() as db:
        ...
        # Merge: existing values as base, only overwrite with non-None incoming values
        merged = {
            ...
            "is_wishlist": game.is_wishlist,
            "wishlist_max_price": game.wishlist_max_price if game.wishlist_max_price is not None else existing_data["wishlist_max_price"],
        }
        ...
        db.execute("""
            ...
            WHERE id = ?
        """, (
            merged["title"], merged["platform_id"], merged["item_type"], merged["barcode"],
            merged["igdb_id"], merged["release_date"], merged["publisher"], merged["developer"],
            merged["genre"], merged["description"], merged["cover_url"], merged["region"],
            merged["condition"], merged["completeness"], merged["location"], merged["purchase_date"],
            merged["purchase_price"], merged["current_value"], merged["notes"],
            1 if merged["is_wishlist"] else 0, merged["wishlist_max_price"], game_id
        ))
        db.commit()
